...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Olá @${num.split('@')[0]}\nBem vindo ao grupo nao seja ghost parceiro (・_・;) *${mdata.subject}*\nPor favor não seja um ghost❤️`
				...
				teks = `Tchau @${num.split('@')[0]} ja tava na hora🐊`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
		...
		try {
			...
			switch(command) {
				...
				case 'info':
					...
					break
				case 'blocklist':
					teks = 'Esta é a lista de números bloqueados,eles mereciam👺 :\n'
	...
	case 'shit':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://images.app.goo.gl/u7oRYByY8KmKjprX8`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'sim'})
					break
	case 'shit':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://images.app.goo.gl/GPJVkpaZrN3xhn6U8`)
				...
				case 'canal':
					...
					break
                case 'mostra a boa bot ':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://i.imgur.com/yOFxSUR.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'fof❤️'})
					break
				case 'mia1':
					memein = await kagApi.memeindo()
